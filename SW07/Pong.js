...
function setUpAttributesAndUniforms() {
  ...
  ctx.uModelViewMatId = gl.getUniformLocation(
    ctx.shaderProgram,
    "uModelViewMat"
  );
  ctx.uSamplerId = gl.getUniformLocation(ctx.shaderProgram, "uSampler");
  ctx.uIsTextureId = gl.getUniformLocation(ctx.shaderProgram, "uIsTexture");
}
...
function drawAnimated(timeStamp) {
  if (first) {
    ...
    if (timeElapsed >= 1) {
      ...
      objCC.rad += 0.1;
      objTC.rad += 0.1;
      if (objCC.rad >= 2 * Math.PI) objCC.rad = 0;
      if (objTC.rad >= 2 * Math.PI) objTC.rad = 0;
    }
  }
  loadTexture();
  window.requestAnimationFrame(drawAnimated);
